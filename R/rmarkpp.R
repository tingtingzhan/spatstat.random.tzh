...
rmarkpp <- function(
  ...
  rfn <- names(dots)
  names(rfn) <- rfn # just easier for developer to debut
  
  par0 <- as.data.frame.list(unlist(dots, recursive = FALSE)) # recycle length
  par <- lapply(rfn, FUN = function(i) { # (i = 'rMatClust')
  ...
  fn <- if (length(par) == 2L) {
    ...
    message(cli_text(sprintf(fmt = 'Point-pattern generated by {.fn %s}', col_blue(rfn[1L]))), appendLF = FALSE)
    message(cli_text(sprintf(fmt = 'Marks generated by {.fn %s}', col_magenta(rfn[2L]))), appendLF = FALSE)
