...
rmarkpp <- function(
  ...
  fn <- if (length(par) == 2L) {
    ...
    # cannot click; not sure why
    #cli_text(sprintf(fmt = 'Point-pattern generated by {.run [%s](spatstat.random::%s())}', rfn[[1L]], rfn[[1L]]))
    #cli_text(sprintf(fmt = 'Marks generated by {.run [%s](stats::%s())}', rfn[[2L]], rfn[[2L]]))
    message(sprintf(fmt = 'Point-pattern generated by %s', col_blue(format_function_(rfn[[1L]]))))
    message(sprintf(fmt = 'Marks generated by %s', col_magenta(format_function_(rfn[[2L]]))))
...
format_function_ <- function(fn) {
  # `fn` is \link[base]{character} scalar
  x <- getAnywhere(fn)
  y <- grep(pattern = '^namespace\\:', x = x$where, value = TRUE)
  if (length(y) != 1L) stop('really?')
  paste0(gsub(pattern = '^namespace\\:', replacement = '', y), '::', fn)
}
